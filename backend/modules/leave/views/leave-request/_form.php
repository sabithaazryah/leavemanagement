...
<div class="leave-request-form form-inline">
        ...
        <div class="row">
                ...
                if ($recommender_exists) {
                        ...
                        $leave_ctaegory = LeaveConfiguration::find()->where(['employee_id' => $model->employee_id])->all();
                        ?>
        ...
        <div class="row">
                ...
                <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
                        ...
                        <?= $form->field($model, 'no_of_days')->textInput(['readonly' => true])->label('Leave Applied (days)') ?>
                </div><input type="hidden" id="leave_exists" value="">

                <input type="hidden" id="appied-leave-dates" value="0">

                <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>    <?= $form->field($model, 'leave_type')->dropDownList(ArrayHelper::map($leave_ctaegory, 'id', 'leavename'), ['prompt' => 'Select']) ?>
...
//                                LeaveTypeValidation(from_date, to_date);
                ...
                $('#leaverequest-leave_type').change(function (e) {
                        LeaveExists();
                });
                ...
                function LeaveExists() {
                        var leave_category = $('#leaverequest-leave_type').val();
                        var leave_count = $('#leaverequest-no_of_days').val();
                        if (leave_count && leave_category) {
                                $.ajax({
                                        type: 'POST',
                                        cache: false,
                                        data: {leave_count: leave_count, leave_category: leave_category},
                                        url: homeUrl + 'leave/leave-request/check-leaves',
                                        success: function (data) {
                                                if (data == 0) {
                                                        alert('This category has no leaves.Please choose another one');
                                                        $('#leaverequest-leave_type').val('');
