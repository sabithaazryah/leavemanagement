...
class LeaveRequestController extends Controller {
        ...
        public function actionCreate() {
                ...
                if ($model->load(Yii::$app->request->post()) && Yii::$app->SetValues->Attributes($model)) {
                        ...
                        $leave_category_details = \common\models\LeaveConfiguration::findOne($model->leave_type);
                        if ($leave_category_details->available_days >= $model->no_of_days) {
                                ...
                                try {
                                        if ($model->save() && $this->LeaveDetails($model)) {
                                                ...
                                                Yii::$app->session->setFlash('error', "Your leave request is not submitted due to some technical reasons. Please try again.");
                                        }
                                } catch (Exception $e) {
                                        $transaction->rollBack();
                                        Yii::$app->session->setFlash('error', "Your leave request is not submitted due to some technical reasons. Please try again.");
                                ...
                                Yii::$app->session->setFlash('error', "The selected leave category doesn't have requested leaves. Please try again.");
                        }
                }
        ...
        public function actionLeaves() {
                if (Yii::$app->request->isAjax) {
                        ...
                        $from_leave_type = '';
                        $to_leave_type = '';
                        if (isset($_POST['single_date_type']) && $_POST['single_date_type'] != '') {
                                $single_leave_type = $_POST['single_date_type'];
                        }
                        if (isset($_POST['from_leave_type']) && $_POST['from_leave_type'] != '') {
                                $from_leave_type = $_POST['from_leave_type'];
                        }
                        if (isset($_POST['to_leave_type']) && $_POST['to_leave_type'] != '') {
        ...
        /*
         * check the leave category has requested leaves
         */

        public function actionCheckLeaves() {
                $leave_category = $_POST['leave_category'];
                $leave_count = $_POST['leave_count'];
                $leave_category_details = \common\models\LeaveConfiguration::findOne($leave_category);
                if ($leave_category_details->available_days >= $leave_count) {
                        $exist = 1;
                } else {
                        $exist = 0;
                }
